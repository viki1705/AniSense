...
personalization_service = PersonalizationService()
    ...
    def build_user_profile_query(self, user_id: str) -> str:
        ...
        Combines genres, themes, and sample anime to create enhanced search query
        """
        genres = UserPreferencesDB.get_genres(user_id)
        themes = UserPreferencesDB.get_themes(user_id)
        sample_anime = UserPreferencesDB.get_sample_anime(user_id)
        ...
        if sample_anime:
            sample_titles = [a["title"] for a in sample_anime[:2]]
            if sample_titles:
                parts.append(f"similar to {' and '.join(sample_titles)}")
    ...
    def get_personalized_recommendations(
        ...
        2. Build preference-based query
        3. Manage explicit query vs preference query
        4. Search ChromaDB with enhanced query
        5. Filter by user settings
        6. Apply preference boosting
        7. Remove disliked anime
        8. Return top N recommendations
    ...
    def filter_by_user_settings(self, results: List[Dict], settings: Dict) -> List[Dict]:
        ...
        max_episodes = settings.get("max_episodes")
        content_filters = settings.get("content_filters", [])
        preferred_type = settings.get("preferred_type")
        ...
        for anime in results:
            ...
            # Check content filters (simplified - would need more sophisticated filtering)
            # For now, skip if we have specific filter requirements
